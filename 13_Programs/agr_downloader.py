...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download Australian Greyhound Rater Excel reports over a date range using a manual browser login."
    )
...
def target_path_for(day: date, output_dir: Path) -> Path:
    return output_dir / f"AGR_{day.strftime('%Y%m%d')}.xlsx"
...
def install_debug_listeners(page) -> None:
    ...
    def on_response(response) -> None:
        if response.url.startswith("https://agrapi.makebettinggreat.com/"):
