...
LOGIN_URL = "https://ukgr.makebettinggreat.com"
REPORT_URL_TEMPLATE = "https://ukdapi.makebettinggreat.com/v1/race/detail/report/{yyyymmdd}?source=ONE"
DEFAULT_OUTPUT_DIR = "/Volumes/SCANDISK128/Horse Analysis/UK Mutts"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download Greyhound UK Rater reports over a date range using a manual browser login."
    )
...
def ensure_output_dir(path: Path) -> None:
    volume_root = Path("/Volumes/SCANDISK128")
...
def target_path_for(day: date, output_dir: Path) -> Path:
    return output_dir / f"UKD_{day.strftime('%Y%m%d')}.xlsx"
...
def install_debug_listeners(page) -> None:
    ...
    def on_response(response) -> None:
        if response.url.startswith("https://ukdapi.makebettinggreat.com/"):
